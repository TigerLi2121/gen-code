  ...
  <div>
    <t-card class="list-card-container">
      <t-row justify="space-between" class="table-bar">
        <t-col :span="3">
          <div>
            ...
            <div
              v-has="'{{ module_name }}:{{ table_name }}:delete'"
              class="delete-btn"
            >
              <t-button
                variant="base"
      ...
      >
        <template #op="{ row }">
          <a
            v-has="'{{ module_name }}:{{ table_name }}:update'"
            class="t-button-link"
            @click="update(row)"
            >修改</a
          >
      ...
      >
        <template #body>
          <t-form ref="form" :data="d" reset-type="initial" @submit="onSubmit"
            >{{#each attributes
            }}{{#unless (or (or pk (eq column_name "created_at")) (eq column_name "updated_at"))}}
            <t-form-item label="{{ comment }}" name="{{ column_name }}">
              <t-input
                v-model="d.{{ column_name }}"
